...
class AsyncDataLoader:
    def __init__(self, batch_size=32, start_idx=0, end_idx=None, inference_server_url="http://localhost:8000", output_file="embeddings.parquet"):
    ...
    async def get_embeddings(self, image_data: List[str], batch_id: str) -> Tuple[List[np.ndarray], List[int]]:
    ...
    def create_dataset(self):
        ...
        dataset = load_dataset("laion/relaion2B-en-research-safe", streaming=True)['train']
    ...
    async def save_embeddings(self, urls: List[str], embeddings: List[np.ndarray]):
    ...
    async with AsyncDataLoader(batch_size=32) as loader:
        ...
        for batch_idx, batch in enumerate(dataset):
            ...
            if images:
                ...
                embeddings, embed_indices = await loader.get_embeddings(images, f"batch_{batch_idx}")
                
                # Map back to original indices
                valid_indices = [download_indices[i] for i in embed_indices]
                valid_urls = [batch['url'][i] for i in valid_indices]
                
                print(f"Batch {batch_idx}: {len(valid_urls)} successful out of {len(batch['url'])} total")
