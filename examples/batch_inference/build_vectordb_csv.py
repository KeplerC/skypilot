...
def read_csv_from_s3(bucket_name: str, key: str,
...
def process_batch(collection, batch_df):
    ...
    embeddings = batch_df['embedding'].apply(
        lambda x: [float(i) for i in x.split(',')]).tolist()
...
def main():
    parser = argparse.ArgumentParser(
        description='Build ChromaDB from S3 CSV files')
    ...
    parser.add_argument('--prefix',
                        type=str,
                        required=True,
                        help='S3 prefix/folder containing CSV files')
